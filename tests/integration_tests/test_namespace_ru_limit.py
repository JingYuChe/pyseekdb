...
- refresh_config: writing {"ru_limit":{"enabled":0}} into sdk_namespaces.info
...
enabled=0 + a throttle, the tail of a burst stops failing).
"""
...
def _hammer(ns, start: int, n: int) -> list[bool]:
    ...
    for i in range(start, start + n):
        try:
            ns.add(ids=[f"h_{i:06d}"], embeddings=[_emb(i)],
                   documents=[f"d{i}"], metadatas=[{"seq": i}])
...
def _set_info_ru_disabled(admin, collection_id: str, namespace_id: int) -> None:
    _set_info(admin, collection_id, namespace_id, '{"ru_limit": {"enabled": 0}}')
...
class TestNamespaceRuLimit:
    ...
    def test_tps_throttling_kicks_in(self, oceanbase_client):
        ...
        try:
            ...
            assert n_blocked > 0, (
                f"expected some writes throttled by TPS limit, got 0/{len(blocked)} "
                "(RU limiter not enforcing?)"
            )
        finally:
    ...
    def test_refresh_config_from_info_disables_limit(self, oceanbase_client):
        """enabled=0 in sdk_namespaces.info, loaded lazily on the first throttle, lifts the limit.

        Note the limiter's 2-minute refresh cooldown: only the FIRST throttle on a fresh
        controller refreshes (last_refresh_time==0). So enabled=0 must already be in the
        ...
        try:
            ...
            # refresh_config -> reads enabled=0 -> buckets unlimited -> the rest all pass.
            blocked = _hammer(ns, 0, 300)
            assert sum(blocked) >= 1, (
                "expected the default limit to trip at least once (which triggers refresh)"
            )
            tail = blocked[-100:]
            assert sum(tail) == 0, (
                f"after refresh loaded enabled=0 the tail must stop throttling; "
    ...
    def test_refresh_config_numeric_override_lifts_limit(self, oceanbase_client):
        """Large qps/tps burst+refill in info.ru_limit (not enabled=0) also lifts the limit."""
        owner = oceanbase_client
        admin = _raw_client()
        ...
        try:
            ...
            ns_id = int(ns.namespace_id)
            # numeric field overrides (enabled stays 1) — proves per-field parsing, not just enabled.
            _set_info(admin, coll.id, ns_id,
                      '{"ru_limit": {"enabled": 1, "qps_burst": 1000000, "qps_refill": 1000000, '
                      '"tps_burst": 1000000, "tps_refill": 1000000}}')
            blocked = _hammer(ns, 0, 300)
            assert sum(blocked) >= 1, "default limit should trip once to trigger refresh"
            tail = blocked[-100:]
            assert sum(tail) == 0, (
                f"large tps/qps override must lift the limit; tail_blocked={sum(tail)}/{len(tail)}"
            )
        finally:
            time.sleep(6)
            owner.delete_collection(name=name)
    ...
    def test_refresh_config_malformed_info_is_safe(self, oceanbase_client):
        ...
        try:
            ...
            ns_id = int(ns.namespace_id)
            # ru_limit is a string, not an object -> JSON_EXTRACT('$.ru_limit.enabled') is NULL
            ...
            assert sum(blocked) > 0, (
                "malformed config must fall back to the default limit (still throttling), "
                f"got 0/{len(blocked)} blocked"
            )
        finally:
            time.sleep(6)
            owner.delete_collection(name=name)
